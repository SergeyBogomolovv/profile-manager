...
func NewGRPCController(svc ProfileService) *gRPCController {
	validate := validator.New()
	return &gRPCController{svc: svc, validate: validate}
}
...
func (c *gRPCController) GetProfile(ctx context.Context, req *pb.GetProfileRequest) (*pb.ProfileResponse, error) {
	...
	if err != nil {
		...
		logger.Extract(ctx).Error("failed to get profile", "error", err)
...
func (c *gRPCController) UpdateProfile(ctx context.Context, req *pb.UpdateProfileRequest) (*pb.ProfileResponse, error) {
	...
	if err != nil {
		...
		logger.Extract(ctx).Error("failed to update profile", "error", err)
